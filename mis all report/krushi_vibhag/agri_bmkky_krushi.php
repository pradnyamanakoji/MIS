<?php
include 'db.php'; // your mysql connection with $con
...
$sections = [
    "construction_of_new_well_2" => "Construction of New Well",
    "diesel_engine_2" => "Diesel Engine",
    "electricity_connection_charges_2" => "Electricity Connection Charges",
    "inwell_boring_2" => "Inwell Boring",
    "kitchen_garden_2" => "Kitchen Garden",
    "lining_to_farm_pond_2" => "Lining to Farm Pond",
    "pumpset_2" => "Pumpset",
    "reparing_of_old_well_2" => "Repairing of Old Well",
    "solar_pump_2" => "Solar Pump"
];
...
$columns_display = [
    ...
    "bdo_scrutiny_site_inspection_report_estimate" => "BDO Scrutiny Site Inspection Report & Estimate",
    "ado_admin_tech_approval_work_order_presanction" => "ADO Admin. Tech Approval/Work Order/PreSanction",
    "ado_construction_bill_approved_for_payment" => "ADO Construction Bill Approved For Payment",
    "ado_construction_bill_disbursed" => "ADO Construction Bill Disbursed",
    "payment_in_process" => "Payment In Process",
    "amount_discharged_12012024" => "Amount Discharged 12-01-2024",
    "amount_discharged_05012024" => "Amount Discharged 05-01-2024",
    "amount_discharged_22122023" => "Amount Discharged 22-12-2023",
    "amount_discharged_15122023" => "Amount Discharged 15-12-2023",
    "amount_discharged_11122023" => "Amount Discharged 11-12-2023",
    "amount_discharged_08122023" => "Amount Discharged 08-12-2023"
];
...
session_start();
$header_title = "Birsa Munda Krishi Kranti Yojana (Outside Tribal Sub Plan) 2022-2023 - Form 3";
...
if (isset($_POST['save_section']) && isset($_POST['section_table'])) {
    ...
    foreach ($talukas as $taluka) {
        ...
        foreach (array_keys($columns_display) as $col) {
            ...
            if (strpos($col, "amount_discharged") === 0) {
        ...
        $check_sql = "SELECT taluka FROM $table WHERE taluka = '$taluka_safe'";
        ...
        if (mysql_num_rows($check_res) > 0) {
            // Update, also update updated_at to current timestamp
            $set_parts = [];
            ...
            $update_sql = "UPDATE $table SET $set_sql WHERE taluka = '$taluka_safe'";
            $update_res = mysql_query($update_sql, $con);
            if (!$update_res) {
                die("Update failed: " . mysql_error());
            }
        } else {
            // Insert (updated_at will default automatically)
            $cols = "taluka, " . implode(", ", array_keys($data));
...
// Fetch data with updated_at filtering
function fetch_data_filtered($table, $from, $to, $con) {
    $from_safe = mysql_real_escape_string($from);
    $to_safe = mysql_real_escape_string($to);
    $sql = "SELECT * FROM $table WHERE updated_at BETWEEN '$from_safe 00:00:00' AND '$to_safe 23:59:59'";
...
function export_excel($table, $con, $columns_display, $talukas, $section_title) {
    $data_rows = fetch_data_filtered($table, null, null, $con);
...
function export_pdf($table, $con, $columns_display, $talukas, $section_title) {
    $data_rows = fetch_data_filtered($table, null, null, $con);
    header("Content-Type: application/pdf");
    header("Content-Disposition: attachment; filename={$table}_report.pdf");
...
if (isset($_GET['export_type'], $_GET['table'])) {
    ...
    if ($export_type === 'excel') {
        export_excel($export_table, $con, $columns_display, $talukas, $sections[$export_table]);
    } elseif ($export_type === 'pdf') {
        export_pdf($export_table, $con, $columns_display, $talukas, $sections[$export_table]);
    } else {
